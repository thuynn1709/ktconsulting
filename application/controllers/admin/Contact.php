...
class Contact extends MY_Controller {
    ...
    public function index() {
        ...
        $total_row = $this->contact_model->count_all_results($search);
        ...
        $data["results"] = $this->contact_model->get_all($search, $config["per_page"], $offset);
        $data["links"] = $this->pagination->create_links();

        // View data according to array.
        
        $data['total'] = $this->contact_model->count_all();
        $data['count_new_email'] = $this->contact_model->count_new_email();
    ...
    public function readmail () {
        ...
        $contact = $this->contact_model->get_one($id);

        if (count((array) $contact) == 0) {
            redirect('admin/contact/index');
        }
        
        $this->contact_model->update_readed($id);
        $data['total'] = $this->contact_model->count_all();
        $data['count_new_email'] = $this->contact_model->count_new_email();
        $data['result'] = $contact;
        $this->load->view('admin/contact/readmail', $data);
        $this->_loadAdminFooter();
    }
    ...
    public function creat () {
        ...
        $data['total'] = $this->contact_model->count_all();
        $data['count_new_email'] = $this->contact_model->count_new_email();
        $this->load->view('admin/contact/creat', $data);
        $this->_loadAdminFooter();
    }
